    ...
    <main className="bg-bg text-text">
      <section className="bg-white border-b border-border">
        <div className="mx-auto max-w-6xl px-4 py-16 sm:px-6 lg:px-8 lg:py-20">
          <h1 className="text-4xl sm:text-5xl font-bold text-brand-900">
            {about.hero.title}
          </h1>
          ...
          <div className="grid gap-8 lg:grid-cols-[1.15fr_0.85fr]">
            <div className="rounded-3xl border border-border bg-white p-8 shadow-sm">
              ...
              <div className="mt-5 space-y-5">
                {about.intro.paragraphs.map((p, idx) => (
                  <p key={idx} className="text-[15px] leading-8 text-muted">
                    {p}
                  </p>
                ))}
              </div>
            ...
            <div className="grid gap-6">
              <div className="rounded-3xl bg-brand text-white p-7 shadow-sm">
                <h3 className="text-xl font-bold">{about.vision.title}</h3>
          ...
          <div className="mt-8 grid gap-6 lg:grid-cols-3">
            {about.approach.steps.map((step, idx) => (
              ...
              >
                ...
                {step.note ? (
                  <p className="mt-4 text-sm leading-8 text-brand">
                    {step.note}
                  </p>
                ) : null}
              </div>
